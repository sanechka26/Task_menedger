...
class TaskManagerApp:
    ...
    def Entrance(self):
        ...
        c.execute("SELECT * FROM users WHERE login=? AND password=?", (login, hashed_password))
        user = c.fetchone()
        conn.close()
    ...
    def registration(self):
        ...
        if not login or not password or not confirm_password:
            ...
            hashed_password = self.hash_password(password)
            try:
                c.execute("INSERT INTO users (login, password) VALUES (?, ?)", (login, hashed_password))
                conn.commit()
                messagebox.showinfo(title='Успех', message='Регистрация прошла успешно!')
                self.registration_window.destroy()  # Закрытие окна регистрации
